...
const createReactCourse = async () => {
  try {
    ...
    // Check if React course already exists
    let course = await Course.findOne({ courseCode: 'REACT101' });
    
    if (course) {
      console.log('\n📚 React course already exists. Updating content...');
    } else {
      console.log('\n📚 Creating new React JS course...');
      course = new Course({
        courseCode: 'REACT101',
    ...
    console.log('\n✅ React JS course created/updated successfully!\n');
    ...
    console.log('='.repeat(60));
    console.log('\n🎉 Students enrolled in this course can now access all content!');
