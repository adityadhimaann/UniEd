...
const addReactContent = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('Connected to MongoDB\n');

    // Find React course (search by name containing "react")
    const course = await Course.findOne({ 
      courseName: { $regex: /react/i } 
    }).populate('faculty', 'firstName lastName');

    if (!course) {
      console.log('❌ No React course found in database');
      console.log('Please provide the exact course name or code');
      process.exit(1);
    }
    ...
    console.log('='.repeat(60));
    console.log('\n🎉 Students enrolled in this course can now access all content in the frontend!');
    console.log('\n💡 To view: Login as an enrolled student → Open course → Click "Content" tab');
