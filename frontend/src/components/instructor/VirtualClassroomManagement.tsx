    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={fetchAllVirtualClasses} disabled={loading}>
          ...
          {virtualClasses.map((vc) => (
            <Card key={vc._id} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">{vc.course?.courseCode} - {vc.course?.courseName}</p>
              ...
              <CardContent className="space-y-3">
                {vc.description && <p className="text-sm text-muted-foreground line-clamp-2">{vc.description}</p>}
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2">
                    <Calendar className="h-4 w-4" style={{ color: 'white' }} />
                    <span>{format(new Date(vc.scheduledStartTime), 'MMM dd, yyyy')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4" style={{ color: 'white' }} />
                    <span>{format(new Date(vc.scheduledStartTime), 'h:mm a')} - {format(new Date(vc.scheduledEndTime), 'h:mm a')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="pt-2 flex gap-2">
                  ...
                  {vc.status === 'live' && (
                    <>
                      ...
                      <Button onClick={() => handleEndClass(vc._id)} variant="destructive" className="flex-1">
                        <StopCircle className="h-4 w-4 mr-2" />End
                      </Button>
                    </>
                  )}
                  {vc.status === 'ended' && (
                    <Button onClick={() => handleDeleteClass(vc._id)} variant="outline" className="flex-1">
          ...
          <form onSubmit={handleCreateClass} className="space-y-4">
            <div>
              ...
              <Input id="title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                placeholder="e.g., React Fundamentals Q&A" required />
            </div>
            <div>
